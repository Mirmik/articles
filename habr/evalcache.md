...
Декоратор может быть применен как к функциям, так и к объектам других типов. В этот момент на их основе строится ленивый объект с хэшключем, расчитанным на основе репрезентации (или с помощью специально определенной для данного типа хэшфнкции).
...
Поскольку хэши объектов строятся на основе данных на пораждающих эти объекты аргументах, при изменении аргумента хэш объекта изменяется и вместе с ним каскадно изменяются хэши всех базирующихся на нем объектах. Это позволяет поддерживать данные кэша в актуальном состоянии, вовремя производя обновление.

Ленивые объекты выстраиваются в дерево. Если мы производим операцию unlazy над одним из объектов, будет загружено и пересчитано ровно столько объектов, сколько необъодимо для восстановления справедливости. В идеале - просто загрузится необходимый объект. В этом случае алгоритм не будет даже подтягивать в память образующие объекты. 
...
evalcache - часть проекта zencad. Это маленький скриптовый кадик, вдохновленный и эксплуатирующий теже идеи, что и openscad. В отличие от mesh ориентированного openscad в zencad, работающем на ядре opencascade используется brep представление объектов, а скрипты пишутся на языке python.
...
Пересечение резьбовых поверхностей - сложная расчетная задача. Практической ценности, впрочем никакой, кроме проверки математики. Вычисление занимает полторы миинуты. Достойная цель для ленификации. 
</spoiler>
