...
Я постарался показать, что феноменология сознания может быть объяснена без привличения гипотезы квалиа. Изначальная предпосылка вывода строится на постулате "существует, потому что не может не существовать". Возражения к ней могут быть сведены к вопросу "может ли что-то возникнуть из ничего", который не уникален для этой темы и является одним основополагающих филосовских вопросов.
